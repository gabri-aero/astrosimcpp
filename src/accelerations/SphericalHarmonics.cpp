...
SphericalHarmonics::SphericalHarmonics(std::string filename) {
...
math::vector SphericalHarmonics::gravity(const Body& i, const Body& j) {
    ...
    double theta = phi - M_PI/2; // colatitude
    ...
    double dU_dphi = 0;
    double dU_dlam = 0;
    double dU_dr_n, dU_dphi_n, dU_dlam_n;
    ...
    auto acc_body = mu_j/r * (dU_dr * dr_dr_vec + dU_dphi * dphi_dr_vec + dU_dlam * dlam_dr_vec);
